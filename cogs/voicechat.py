...
class VoiceChatCog(commands.Cog):
    ...
    def getVolume(self, ctx):
        return self.bot.getServer(ctx).get("vc_volume", 100) / 100
    ...
    async def playCommand(self, ctx, url: str = None):
        ...
        # get from argument
            ...
            if url.startswith("http") and "youtu" in url and "/" in url:
                if "youtu.be" in url:
                    video_file_name = url.split("/")[-1] + ".mp3"
                else:
                    video_file_name = url.split("watch?v=")[-1] + ".mp3"
